...
- 《TCP/IP详解》里的内容太复杂了，等我以后有空再看吧 。

**拥塞控制的目的就是为了防止过多的包进入链路中，导致链路中的路由器等设备过载而丢弃数据包，引发拥塞。**

TCP协议中，由发送方维护一个反映网络传输能力的的变量叫做**拥塞窗口**(cwnd)。
...
假设起始的拥塞窗口为n，在每接收到一个ACK之后拥塞窗口加1，所以如果网络良好ACK全部按时收到，那么第二轮次的拥塞窗口就变为了2n，之后便是4n，以指数增长。

**慢开始的慢并不是增长速度慢，而是算法初始的拥塞窗口小**，在不清楚网络传输能力的情况下，并不会一下子就设置太大的拥塞窗口。
...
**快恢复就是在快速重传触发慢开始的情况下，使发送效率快速的恢复到一个相对较高的水平。**

按照慢开始和拥塞避免算法，如果快速重传之后也重新执行慢开始算法，因为始终从较小的cwnd开始，所以效率不高。
