...
创建型模式主要关注对象的创建过程，对对象的创建过程进行解耦，拆分创建和使用的逻辑。
...
相对于工厂类而言，建造者模式一般用于比较复杂的对象的创建，此时也只关注一个产品类的创建。
...
结构型模式主要关注类和类之间的关系，主要是有聚合，组合和继承。
...
继承一般意义上都具有强关联型，或者说强侵入性，子类必须继承父类的一系列特性，那么父类的修改必定会影响到子类
