...
只有在上文提到过地**同时关闭**的情况下出现，同时关闭的通信双方在接收到对方的FIN，在发送ACK之后进入到`CLOSING`状态。
...
TCP是基于网络层的IP协议的，但是IP协议并不提供任何可靠传输的保证，所以我们可以任务TCP所处的链路是不可靠的。
...
判断分组是否丢失的依据就是**在一定时间内，是否收到接受的确认信息**。<font size=1>(太粗暴了)</font>
...
连续ARQ协议虽然提高了信道利用率，但是仍然会存在一些问题。	

#### 超时重传

TCP每发送一个报文，都会设定一个**重传计时器**，如果在给定时间之内都没有收到确认消息，就会重传报文。
